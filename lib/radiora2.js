...
var RadioRa2 = function (host, username, password, log) {
    ...
    this.connect = function () {
        ...
        socket.on('data', function (data) {
            ...
            if ((data.indexOf("GNET") == 0) || (data.indexOf("QNET") == 0) || (data.indexOf("QSE") == 0) || data.indexOf("connection established") == 0 || (loggedIn)) incomingData(data);
        }).on('connect', function () {
            // Send a harmless command or ping every 30 seconds
            //TODO: add a configuration option to change keeepalive for QSE vs Main Repeater.
                keepaliveInterval = setInterval(() => {
                        log.info("Sending keepalive request");
                        socket.write('?OUTPUT,7,1\r\n');  // Change to a valid device ID in your system
                        // Or: connection.write('\r\n');
                }, 30000); // 30 seconds
