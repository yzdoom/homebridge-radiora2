...
var RadioRa2 = function (host, username, password, log) {
    ...
    function processIncomingBuffer() {
        var lower = receiveBuffer.toLowerCase();

        if (!usernameSent && lower.indexOf("login:") !== -1) {
            log.debug("Sending username");
            socket.write(username + "\r\n");
            usernameSent = true;
            receiveBuffer = removeThroughToken(receiveBuffer, /login:\s*/i);
            startLoginKick();
            lower = receiveBuffer.toLowerCase();
        }

        if (!passwordSent && lower.indexOf("password:") !== -1) {
            log.debug("Sending password");
            socket.write(password + "\r\n");
            passwordSent = true;
            receiveBuffer = removeThroughToken(receiveBuffer, /password:\s*/i);
            startLoginKick();
            lower = receiveBuffer.toLowerCase();
        }

        if (
            lower.indexOf("connection successful") !== -1 ||
            lower.indexOf("connection established") !== -1
        ) {
            log.debug("Connection success response received; requesting prompt");
            socket.write("\r\n");
            receiveBuffer = receiveBuffer
                .replace(/connection successful\s*/ig, "")
                .replace(/connection established\s*/ig, "");
        }

        var promptMatch = receiveBuffer.match(/(?:GNET|QNET|QSE)>\s*/i);

        if (promptMatch) {
            var promptIndex = receiveBuffer.search(/(?:GNET|QNET|QSE)>\s*/i);
            var beforePrompt = receiveBuffer.substring(0, promptIndex);
            receiveBuffer = receiveBuffer.substring(
                promptIndex + promptMatch[0].length
            );

            if (beforePrompt.trim().length > 0) {
                incomingData(beforePrompt.trim());
            }

            handleReadyPrompt();

            if (receiveBuffer.trim().length > 0) {
                var remaining = receiveBuffer;
                receiveBuffer = "";
                incomingData(remaining.trim());
            }

            return;
        }

        if (usernameSent && !loggedIn) {
            startLoginKick();
        }

        if (loggedIn && receiveBuffer.indexOf("\n") !== -1) {
            var lines = receiveBuffer.split(/\r?\n/);
            receiveBuffer = lines.pop() || "";

            lines.forEach(function (line) {
                if (line.trim().length > 0) {
                    incomingData(line.trim());
                }
            });
        }
    }

    function removeThroughToken(value, tokenRegex) {
        var match = value.match(tokenRegex);

        if (!match) {
            return value;
        }

        return value.substring(match.index + match[0].length);
    }

    function handleReadyPrompt() {
        stopLoginKick();

        if (!loggedIn) {
            loggedIn = true;
            log.info("Login prompt received; connection ready");
            me.emit('loggedIn');
        }
    ...
    function startLoginKick() {
        if (loggedIn || loginKickInterval) {
            return;
        }

        loginKickInterval = setInterval(function () {
            if (
                !loggedIn &&
                usernameSent &&
                socket &&
                ...
                log.debug("Waiting for login prompt; sending Enter");
                socket.write("\r\n");
            }
        }, 1000);
    }

    function stopLoginKick() {
        if (loginKickInterval) {
            clearInterval(loginKickInterval);
            loginKickInterval = null;
        }
    }
    ...
    async function incomingData(data) {
        ...
        // Prompt for data?
        if ((/^GNET>/.test(str)) || (/^QNET>/.test(str)) || (/^QSE>/.test(str))) {
